...
BEGIN {
	...
	use Test::Exception;
    use Test::Warn;
    use_ok('Bio::Root::Biome');
}

ok my $obj = Bio::Root::Biome->new();
isa_ok $obj, 'Bio::Root::Biome';
...
my $verbobj = Bio::Root::Biome->new(-verbose=>1,-strict=>1);
is $verbobj->verbose(), 1;

# THIS WILL NOT BE SUPPORTED!

##$Bio::Root::Root::DEBUG = 1;
#my $seq = Bio::Root::Biome->new(-verbose => 1);
#is $seq->verbose, 1;

# test for bug #1343
my @vals = Bio::Root::Biome->_rearrange([qw(apples pears)],
                                        -apples => 'up the',
                                        -pears  => 'stairs');
...
# class method
{
    local $Bio::Root::Biome::VERSION = 8.9;
    warning_like{ Bio::Root::Biome->deprecated('Test1') } qr/Test1/, 'simple';
    warning_like{ Bio::Root::Biome->deprecated(-message => 'Test2') } qr/Test2/;
    warning_like{ Bio::Root::Biome->deprecated('Test3', 999.999) } qr/Test3/,
        'warns for versions below current version';
    warning_like{ Bio::Root::Biome->deprecated(-message => 'Test4',
                                              -version => 999.999) } qr/Test4/,
        #'warns for versions below current version';
    throws_ok{ Bio::Root::Biome->deprecated('Test5', 0.001) } qr/Test5/,
        'throws for versions above current version';
    throws_ok{ Bio::Root::Biome->deprecated(-message => 'Test6',
                                       -version => 0.001) } qr/Test6/,
        'throws for versions above current version';

    throws_ok{ Bio::Root::Biome->deprecated(-message => 'Test6',
                                           -version => $Bio::Root::Biome::VERSION) } qr/Test6/,
        'throws for versions equal to current version';
    
    # object method
    my $root = Bio::Root::Biome->new();
...
# TODO: _set_from_args() probably should be paramterized and should NOT be in RootI
...

{
    ...
    # TODO: fix this to be more moose-y
    sub new {
        my $class = shift;
        my $self = {};
        bless $self, ref($class) || $class;
...
can_ok ('Bio::Foo3','t5');
ok ! UNIVERSAL::can('Bio::Root::Biome','t5'), "Methods don't pollute original Bio::Root::Root namespace";
