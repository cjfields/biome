...
role {
    my $p = shift;
    my %class_map = %{$p->delegates};
    while (my ($accessor, $delegate_map) = each %class_map) {
        die("Must define 'isa' for $accessor") unless
...
Biome::Role::Delegator - <One-line description of module's purpose>

=head1 VERSION

This documentation refers to Biome::Role::Delegator version MooseX::Role::Parameterized.

=head1 SYNOPSIS

   with 'Biome::Role::Delegator';
