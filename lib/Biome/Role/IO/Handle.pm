package Biome::Role::IO::Handle;

use Biome::Role;

use Biome::Type::IO qw(IO_Stream IO_Mode);
...
has mode => (
    ...
    default => sub {
        my $self = shift;
        # this only works if the fh is writable
        my @layers = PerlIO::get_layers($self->fh, output => 1);
        @layers ? 'w' : 'r';
    }
);
...
sub close {
    ...
    if( defined( my $fh = $self->fh )) {
        ...
        # TODO: use of IO::String isn't implemented yet, since one can now do
        # this directly with '(open my $foo, '<', \$string)'
...
Biome::Role::IO::Base - <One-line description of module's purpose>

=head1 VERSION

This documentation refers to Biome::Role::IO::Base version Biome::Role.

=head1 SYNOPSIS

   with 'Biome::Role::IO::Base';
